...
function Galeria() {
  ...
  const prevImage = () => setSelectedIndex((i) => (i === 0 ? imagenes.length - 1 : i - 1));
  const nextImage = () => setSelectedIndex((i) => (i === imagenes.length - 1 ? 0 : i + 1));
  ...
  async function handleEditSubmit(e) {
    ...
    try {
      ...
      await axios.put(`http://localhost:4000/api/imagenes/${editData.id}`, form, {
        headers: {
    ...
    <div className="container my-4">
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <div>
          {user ? (
            <>
              <span className="me-2">Hola, {user.nombre} ({user.rol})</span>
              <button className="btn btn-sm btn-outline-secondary me-2" onClick={logout}>Cerrar sesión</button>
            </>
          ) : (
      ...
      <form className="row g-2 mb-3" onSubmit={(e) => e.preventDefault()}>
        ...
        <div className="col-md-3">
          ...
          <input name="q" className="form-control" value={filters.q} onChange={handleFilterChange} />
        </div>
      ...
      {/* GRID */}
      <div className="row">
        {imagenes.length === 0 && <div className="col-12"><p>No hay imágenes.</p></div>}
        {imagenes.map((img, idx) => (
          <div key={img.id} className="col-12 col-sm-6 col-md-4 col-lg-3 mb-4">
            <div className="card h-100 shadow-sm">
              <img src={img.url} alt={img.titulo} className="card-img-top" style={{ objectFit: "cover", height: "180px", cursor: "pointer" }} onClick={() => openDetail(idx)} />
              <div className="card-body">
                <h5 className="card-title">{img.titulo}</h5>
                <p><strong>Autor:</strong> {img.autor || "Anónimo"}</p>
                <p><strong>Categoría:</strong> {img.categoria || "Sin categoría"}</p>
                <p className="text-muted">{img.descripcion?.slice(0, 80)}</p>

                <div className="d-flex justify-content-between mt-2">
                  <div>
                    <button className="btn btn-outline-primary btn-sm me-2" onClick={() => openDetail(idx)}>Ver</button>
                    {isAdmin && <button className="btn btn-outline-warning btn-sm me-2" onClick={() => openEditModal(img)}>Editar</button>}
                    {isAdmin && <button className="btn btn-outline-danger btn-sm" onClick={() => handleDelete(img.id)}>Eliminar</button>}
                  </div>
                  <small className="text-muted">{new Date(img.creado_en).toLocaleDateString()}</small>
                </div>
          ...
          <div className="modal-dialog modal-lg modal-dialog-centered">
            <div className="modal-content">
              ...
              <div className="modal-body">
                <img src={imagenes[selectedIndex].url} alt={imagenes[selectedIndex].titulo} className="img-fluid mb-3" />
              ...
              <div className="modal-footer">
                <button className="btn btn-secondary" onClick={prevImage}>Anterior</button>
                <button className="btn btn-secondary" onClick={nextImage}>Siguiente</button>
